...
export function transformBookingsData(queryResult: QueryResult): BookingListItem[] {
  ...
  return bookings.map((booking) => {
    ...
    return {
      ...
      customer_loyalty_tier: (customer?.loyalty_tier as 'bronze' | 'silver' | 'gold' | 'platinum' | null) || null,
      ...
      fare_amount: (() => {
        const basePrice = bookingPricing?.price ? Math.round(parseFloat(bookingPricing.price) * 100) : 0;
