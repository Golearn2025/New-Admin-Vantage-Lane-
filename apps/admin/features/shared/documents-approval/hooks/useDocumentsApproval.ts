...
import { getDocumentCounts, listDocuments } from '@entities/document';
...
export function useDocumentsApproval() {
  const { user } = useCurrentUser(); // ✅ RBAC: Get current user for organization filtering
  const [documents, setDocuments] = useState<Document[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  const [filters, setFilters] = useState<DocumentsApprovalFilters>({
    tab: 'all', // ✅ RBAC: Show ALL documents by default for operators
    search: '',
    status: 'all',
    documentType: 'all',
    category: 'all',
  ...
  const refetch = async () => {
    await Promise.all([fetchDocuments(), fetchCounts()]);
  };
