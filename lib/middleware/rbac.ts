 ...
 */
export async function checkAdminAccess(options: RBACOptions = {}): Promise<RBACResult> {
  const { allowedRoles = ['super_admin', 'admin'], requireActive = true } = options;
  ...
  try {
    ...
    if (requireActive && !adminUser.is_active) {
      return {
        ...
        user,
        adminUser,
        error: NextResponse.json({ error: 'Forbidden - Account is inactive' }, { status: 403 }),
      };
    }
