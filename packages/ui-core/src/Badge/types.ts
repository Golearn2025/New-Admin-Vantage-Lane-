...
export type BadgeBookingStatus = 
  | 'draft' 
  | 'confirmed' 
  | 'active' 
  | 'completed' 
...
export type BadgeValueByVariant<T extends BadgeVariant> = 
  T extends 'status' ? BadgeStatus :
  T extends 'booking_status' ? BadgeBookingStatus :
  T extends 'trip_type' ? BadgeTripType :
  T extends 'category' ? BadgeCategory :
  T extends 'service' ? BadgeService :
  T extends 'payment' ? BadgePaymentStatus :
  string;
